...
# Illustrative example for Figure 4:
      # DATA: Artifical data with confounding event simulated in October 2005 
...
# Wide format (artifical data adapted from WONDER, CDC)
flhom <- read.csv("./syg_dat1_artificial.csv") 
# Long format (artifical data adapted from WONDER, CDC)
flcom <- read.csv("./syg_dat2_artificial.csv") 


#----------------------#
# 3. PREPARE DATA (including adding artificial confounding event)
#----------------------#

# 3.1 Simulate an other confounding event for CT and DE adding 50 after time Oct 2005 time 81/2
# 3.1.1 Wide dataframe: flhom
flhom_sub1 <- flhom %>% filter(Effective==0)
flhom_sub2 <- flhom %>% filter(Effective==1) %>%
  mutate(c_hom=c_hom+5)
flhom <- rbind(flhom_sub1, flhom_sub2)
flhom <- flhom %>% arrange(time)
# 3.1.2 Long dataframe: flcom
flcom_sub1 <- flcom %>% filter(case==1)
flcom_sub2 <- flcom %>% filter(case==0 & Effective==1) %>%
  mutate(hom=hom+5)
flcom_sub3 <- flcom %>% filter(case==0 & Effective==0)
flcom <- rbind(flcom_sub1, flcom_sub2, flcom_sub3)
flcom <- flcom %>% arrange(case,time)

# 3.2 Outlier Sept, 2001 = 2753 homicides due to 9/11; remove from the analyses
flhom[33, 7] = NA

# 3.3 Derive homicide rates per 100,000 for both dataframes
...
# Results for these simple ITS models are not explicitly reported in the paper but are used for plotting Figure 4(a)
...
#----------------------#
# Modelling: segmented log-linked Gaussian generalized linear model for monthly homicide rates, 1999-2014
# Results for this replication of the original CITS are explicitly reported in the paper (see 16pg & Figure 4a in the manuscript)
...
# 6.2 Plot & save Figure 4(a) in the manuscript
tiff(file="./Figure4a",width=4000,height=3000,res=600)
...
points(1:192,chom.obs1 ,cex=0.7, pch=16, col="red3")
...
lines(1:1920/10,chom.m.pred11,col="red3", lwd=2)
legend ("topleft", legend=c("Florida", "Comparison states"), col=c("blue3", "red3"), lty=1:1, cex=0.6, bty="n", inset=0.015)
dev.off()
...
# 7.2.2 Simulate an other confounding event for CT and DE adding 50 after time Oct 2005 time 81/2
data_sub1 <- data %>% filter(State!="Connecticut" & State!="Maryland")
data_sub2 <- data %>% filter(State=="Connecticut"|State=="Maryland") %>%
  filter(time<=81) 
data_sub3 <- data %>% filter(State=="Connecticut"|State=="Maryland") %>%
  filter(time>=82) %>%
  mutate(homicide_total=homicide_total+5)
data <- rbind(data_sub1, data_sub2,data_sub3)
data <- data %>% arrange(State, time)

# 7.2.3 Derive homicide rates
...
# 7.2.4 Derive suicide rates as otherwise they lie outside the convex hull
...
# Save treatment and control units for subsequent data analysis & figures
...
#----------------------#
# Modelling: segmented log-linked Gaussian generalized linear model for monthly homicide rates, 1999-2014
# Results are for the extended CITS models using a synthetic control and simulated data are explicitly reported in the paper (see 16pg & Figure 4b in the manuscript)


# 8.1 Simple ITS models -- used to plot Figure 4b
...
# 9.2 Plot & save Figure 4(b) in the manuscript
tiff(file="./Figure4b",width=4000,height=3000,res=600)
par (mar=c(4,4,1.5,1.5))
plot(1:192,flhom.obs,type="n",ylim=c(0.0,1.0),xlab="Year",
     ylab="Rate per 100,000", main= NULL, cex.lab=.75, cex.axis=.65, frame.plot=F,xaxt="n",las=2) 
rect(82,0.0,192,1.0, col=grey(0.9),border=F) 
points(1:192,flhom.obs,cex=0.7, pch=16, col="blue3") 
points(1:192,chom.obs ,cex=0.7, pch=16, col="red3")
axis(1,at=0:16*12,labels=F, cex.lab=.75, cex.axis=.75) 
axis(1,at=0:15*12+6,tick=F,labels=1999:2014, cex.lab=.75, cex.axis=.65)
lines(1:1920/10,chom.m.pred1,col="red3", lwd=2)
lines(1:1920/10,flhom.m.pred1,col="blue3", lwd=2)
legend ("topleft", legend=c("Florida", "Synthetic control"), col=c("blue3", "red3"), lty=1:1, cex=0.6, bty="n", inset=0.015)
dev.off()



#----------------------#
# 10. PLOT FIGURE 4 by combining panels (a) and (b)
#----------------------#

# Plot & save Figure 4 in the manuscript
tiff(file="./Figure4",width=4000,height=6000,res=600)
par (mfrow=c(2,1))
plot(1:192,flhom.obs1,type="n",ylim=c(0.0,1.0),xlab="Year",
     ylab="Rate per 100,000", main= NULL, cex.lab=.75, cex.axis=.65, frame.plot=F,xaxt="n",las=2) 
title(main= "(a)", adj = 0)
