...
gurobifn.V<- function (variables.v = stop("variables.v missing"), X0.scaled = stop("X0.scaled missing"), 
                       X1.scaled = stop("X1.scaled missing"), Z0 = stop("Z0 missing"), 
                       Z1 = stop("Z1 missing"),psdtol=1e-4,
                       BarConvTol=1e-8,BarIterLimit=1e5,...) 
{
  ...
  modelreg<-list(
    ...
    obj = c,
    objcon = 0,
    Q =  t(X0.scaled) %*% V %*% (X0.scaled)
    )
  
    res <- gurobi(modelreg,params=list(OutputFlag=0,PSDTol=psdtol,BarConvTol=BarConvTol,BarIterLimit=BarIterLimit))
    solution.w <- as.matrix(res$x)
...
###############################
#DEFINING SYNTH FUNCTION
###############################


gurobisynth<- function (data.prep.obj = NULL, X1 = NULL, X0 = NULL, Z0 = NULL, 
  ...
  if (is.null(custom.v) & nrow(X0) != 1) {
    ...
    if (genoud == TRUE) {
      ...
      cat("\n****************", "\n genoud() requested for optimization\n", 
          "\n")
      rgV.genoud <- genoud(gurobifn.V, nvarsV, X0.scaled = X0.scaled, 
    ...
    rgV.optim.1 <- optimx(par = SV1, fn = gurobifn.V, gr = NULL, 
                          hess = NULL, method = optimxmethod, itnmax = NULL, 
                          hessian = FALSE, control = list(kkt = FALSE, starttests = FALSE, 
                                                          dowarn = FALSE, all.methods = all.methods), 
                          X0.scaled = X0.scaled, X1.scaled = X1.scaled, Z0 = Z0, 
                          Z1 = Z1, quadopt = quadopt, margin.ipop = Margin.ipop, 
    ...
    Zall <- cbind(Z1, Z0)
    Beta <- try(solve(t(Xall) %*% Xall) %*% t(Xall) %*% 
                  t(Zall), silent = TRUE)
    if (inherits(Beta, "try-error")) {
      ...
      SV2 <- diag(V)
      SV2 <- SV2/sum(SV2)
      rgV.optim.2 <- optimx(par = SV2, fn = gurobifn.V, gr = NULL, 
                            hess = NULL, method = optimxmethod, itnmax = NULL, 
