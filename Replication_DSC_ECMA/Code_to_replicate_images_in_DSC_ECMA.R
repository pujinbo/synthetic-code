...
# To run the replication, open the .Rproj, then run the below code
...
packages_load <- c("haven", "base", "data.table", "latex2exp", "CVXR",  # used to compute the weights using the alternative using mixtures of CDF
                   "here", "dplyr", "pracma", "quadprog", "parallel", "doSNOW")
...
df <- read_dta(paste0("mw_annual_lagsleads_1974_2014.dta"))
...
for(yy in 1:7){
  ...
  objective <- cvxr_norm((CDF.matrix[,2:ncol(CDF.matrix)] %*% theweights - CDF.matrix[,1]), p = 1)
...
Weights_DiSCo_avg <- (1/length(1:5)) * Weights_DiSCo_avg
Weights_mixture_avg <- (1/length(1:5)) * Weights_mixture_avg
...
weights_df <- data.frame(weights_disco = Weights_DiSCo_avg, weights_mix = Weights_mixture_avg, fips = helpcont)

# merge with built-in state fips codes
weights_df <- merge(weights_df, maps::state.fips, by = "fips")

# disco largest weights
setorder(weights_df, -weights_disco)
head(weights_df)

# mixture largest weights
setorder(weights_df, -weights_mix)
head(weights_df)
...
# run the permutation test in parallel (takes ~3 mins on a 2022 Macbook Pro with Apple M1 Pro chip)
permutation.test.results <- DiSCo_per(controls.per, target.per, T0 = 5,
